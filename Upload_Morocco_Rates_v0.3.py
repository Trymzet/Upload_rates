...
#for testing
try:
    remove("Morocco Rates\MOROCCO_RATES\MOROCCO_RATES_2017-03-30.xlsx")
    remove("Other rates\TURKEY_RATES\TURKEY_RATES_2017-03-30.xlsx")
except:
    pass
...
def load_xml(rates_url, country_abbreviation):
    try:
        ...
        rates_xml = urllib.request.urlopen(rates_url)
...
try:
    with pd.ExcelWriter(MA_output_path, engine="openpyxl") as writer:
        MA_header.to_excel(writer, index=False, header=False)
...
try:
    with pd.ExcelWriter(TR_output_path, engine="openpyxl") as writer:
        TR_header.to_excel(writer, index=False, header=False)
...
SK_etree = ET.fromstring(SK_rates_string)
...
SK_excel_date_format = (SK_effective_date - datetime.datetime(1899, 12, 31)).days + 1
...
# create the final xlsx TODO write a function for this, from here
try:
    with pd.ExcelWriter(SK_output_path, engine="openpyxl") as writer:
        SK_header.to_excel(writer, index=False, header=False)
        SK_data.to_excel(writer, index=False, header=False, startrow=2)
except:
    print("Unable to generate TR rates. :(")

format_date_to_excel(SK_output_path)

print("SK rates generated :)")
#TODO to here
